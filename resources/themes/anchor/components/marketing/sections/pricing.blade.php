...
<section id="pricing">
    <div class="text-center max-w-3xl mx-auto mb-12">
        ...
        <p class="text-[length:var(--text-lg)] text-[color:var(--color-ink-mute)]">
    ...
    <div x-data="{ billing: 'monthly' }" class="max-w-6xl mx-auto">
        ...
        <div class="flex justify-center mb-12">
            <div class="inline-flex p-1 rounded-full bg-surface border border-rule">
                <button
                    x-on:click="billing = 'monthly'"
                    x-bind:class="billing === 'monthly' ? 'bg-accent text-ink' : 'text-[color:var(--color-ink-mute)] hover:text-[color:var(--color-ink)]'"
                    class="px-6 py-2 rounded-full text-sm font-semibold transition-colors">
                    {{ __('landing.pricing.monthly') }}
                </button>
                <button
                    x-on:click="billing = 'yearly'"
                    x-bind:class="billing === 'yearly' ? 'bg-accent text-ink' : 'text-[color:var(--color-ink-mute)] hover:text-[color:var(--color-ink)]'"
                    class="px-6 py-2 rounded-full text-sm font-semibold transition-colors">
        ...
        <div class="grid md:grid-cols-3 gap-6">
            @foreach($plans as $plan)
                ...
                <div class="relative p-8 rounded-2xl bg-surface border {{ $isDefault ? 'border-accent' : 'border-rule' }} flex flex-col">
                    @if($isDefault)
                        <div class="absolute -top-3 left-1/2 -translate-x-1/2 px-4 py-1 rounded-full bg-accent text-ink text-xs font-bold uppercase tracking-wide">
                    ...
                    @endphp
                    <h3 class="font-display text-2xl font-bold text-ink mb-2">{{ $plan->name }}</h3>
                    <p class="text-sm text-[color:var(--color-ink-mute)] mb-6 min-h-[40px]">{{ \Illuminate\Support\Facades\Lang::has($planKey) ? __($planKey) : $plan->description }}</p>

                    <div class="mb-6">
                        <div x-show="billing === 'monthly'" class="flex items-baseline">
                            <span class="font-display text-5xl font-bold text-ink">{{ $plan->currency }}{{ $plan->monthly_price }}</span>
                            <span class="text-[color:var(--color-ink-mute)] ml-2">{{ __('landing.pricing.per_month') }}</span>
                        </div>
                        <div x-show="billing === 'yearly'" x-cloak class="flex items-baseline">
                            <span class="font-display text-5xl font-bold text-ink">{{ $plan->currency }}{{ $plan->yearly_price }}</span>
                            <span class="text-[color:var(--color-ink-mute)] ml-2">{{ __('landing.pricing.per_year') }}</span>
                        </div>
                    </div>
                    ...
                    @if($isCurrent)
                        <div class="block text-center px-6 py-3 rounded-lg bg-[color:var(--color-paper-3)] border border-rule text-[color:var(--color-ink)] font-bold">
                    ...
                    @elseif($authUser)
                        <form method="POST" action="{{ url('/pricing/checkout') }}">
                            ...
                            <button type="submit"
                                class="w-full text-center px-6 py-3 rounded-lg {{ $isDefault ? 'btn-accent' : 'btn-ghost' }} text-ink font-bold transition-colors">
                                {{ __('landing.pricing.cta') }}
                            </button>
                        </form>
                    @else
                        <a href="{{ url('/auth/login') }}?redirect={{ urlencode('/pricing') }}"
                           class="block text-center px-6 py-3 rounded-lg {{ $isDefault ? 'btn-accent' : 'btn-ghost' }} font-bold">
